...
${params.domSnapshot.substring(0, 15000)}`;
      ...
      console.error('Claude API analysis failed, returning mock fallback:', err);
      return this.generateMockAnalysis(params);
    }
  }
...
${params.userSteps.map((step, idx) => `${idx + 1}. ${step}`).join('\n')}`;
      ...
      console.error('Claude API test gen failed, returning mock fallback:', err);
      return this.generateMockTest(params);
    }
  }
...
Keep responses concise but comprehensive.`;
      ...
      console.error('Claude API chat failed, returning mock fallback:', err);
      return this.generateMockChat(prompt);
    }
  }

  private parseJSONResponse(text: string): PageAnalysisResult {
    try {
      const cleaned = text.trim().replace(/^```json\n|```$/g, '').trim();
      return JSON.parse(cleaned) as PageAnalysisResult;
    } catch {
  ...
  private generateMockAnalysis(params: PageAnalysisParams): PageAnalysisResult {
    ...
    // Generate realistic-looking mock bugs depending on URL
    ...
    // Default mock bugs to show off capability
    if (bugs.length === 0) {
      bugs.push({
  ...
  private generateMockTest(params: TestGenParams): TestGenResult {
    // Generate a basic, valid Playwright script replicating the steps
