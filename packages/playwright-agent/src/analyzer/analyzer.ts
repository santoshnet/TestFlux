import { PageSnapshot, BugReport } from '../types';

export class Analyzer {
  async analyze(snapshot: PageSnapshot, aiProvider?: any): Promise<BugReport[]> {
      ...
      try {
        ...
        if (aiResult && Array.isArray(aiResult.bugs)) {
          for (const aiBug of aiResult.bugs) {
            ...
            const isDup = bugs.some((b) => 
              b.category === aiBug.category && 
